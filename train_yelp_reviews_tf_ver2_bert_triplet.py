...
while n_iter < maximum_iter:
    ...
    tmp_seq_anc[:, 0] = CLS_token
    tmp_seq_pos[:, 0] = CLS_token
    tmp_seq_neg[:, 0] = CLS_token
    ...
    for n_index in range(batch_size):
        ...
        tmp_p_tok1 = train_data[tmp_index1][1]
        tmp_p_tok2 = train_data[tmp_index2][1]
        tmp_p_idx1 = [word_2_idx.get(
            x, UNK_token) for x in tmp_p_tok1]
        tmp_p_idx2 = [word_2_idx.get(
    ...
    if n_iter % display_step == 0:
        ...
        tmp_in_pos = train_data[sample_pos[0]][1]
        tmp_in_neg = train_data[sample_neg[0]][1]
        
        tmp_p_pos = [word_2_idx.get(
            x, UNK_token) for x in tmp_in_pos]
        tmp_p_neg = [word_2_idx.get(
            x, UNK_token) for x in tmp_in_neg]
        ...
        print("Positive:", " ".join(tmp_in_pos))
        print("Positive Dist:", str(pos_dist.numpy()))
        print("Negative:", " ".join(tmp_in_neg))
