...
for n_batch in range(tot_batch):
    ...
    tmp_seq_emb = tf.reduce_mean(bert_model(
...
while True:
    ...
    if tmp_phrase == "":
        ...
        tmp_test = tf.reduce_mean(bert_model(
            tmp_anchor_idx, training=False)[1], axis=1)
        tmp_test = tmp_test.numpy()
        
        # Normalize the embeddings to have unit norm. #
        tmp_norm = np.sqrt(np.sum(
            np.square(tmp_test), axis=1, keepdims=True))
        tmp_norm = np.divide(tmp_test, tmp_norm)

        doc_l2_norm  = np.sqrt(np.sum(np.square(
            doc_embeddings), axis=1, keepdims=True))
        doc_vec_norm = np.divide(doc_embeddings, doc_l2_norm)
        
        tmp_sim  = np.sum(
            tmp_norm * doc_vec_norm, axis=1)
        tmp_dist = np.mean(np.square(
            tmp_test - doc_embeddings), axis=1)
        
        # Show the top-k best matches. #
        tmp_sort_idx_1 = list(np.argsort(tmp_dist))
        tmp_sort_idx_2 = list(np.argsort(-tmp_sim))

        for n_display in range(topk):
            tmp_idx_display = tmp_sort_idx_1[n_display]
