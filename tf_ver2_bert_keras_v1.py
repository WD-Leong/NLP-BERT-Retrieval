...
class BERT(tf.keras.Model):
    ...
    def call(self, x_encode, training=True):
        # Extract the vocabulary embedding matrix. #
        x_vocab = tf.range(self.vocab_size)
        w_embed = self.bert_model.enc_embed(x_vocab)
        
        enc_outputs = self.bert_model(
            x_encode, training=training)
        enc_logits  = tf.matmul(
            enc_outputs, w_embed, transpose_b=True)
        return enc_logits, enc_outputs
    
    def infer(self, x):
        tmp_embed = self.call(x, training=False)[1]
        return tmp_embed
    
    def infer(self, x):
        tmp_logits  = self.call(x, training=False)[0]
